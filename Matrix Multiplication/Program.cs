...
namespace Matrix_Multiplication {
    class Program {
        ...
        private static void MatrixMultiplication(int[,] matrix1, int[,] matrix2) {
            ...
            for (int i = 0; i < result.GetLength(0); i++) { //for each column in the results matrix
                for (int j = 0; j < result.GetLength(1); j++) { //for each row in the results matrix
                    for (int k = 0; k < matrixA.GetLength(1); k++) { //iterate over every element in a single row of MatrixA
